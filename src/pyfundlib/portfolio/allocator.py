...
class BaseAllocator(ABC):
    ...
    @abstractmethod
    def allocate(
        self, signals: pd.Series, data: Optional[pd.DataFrame] = None
    ) -> AllocationResult:
...
class PortfolioAllocator(BaseAllocator):
    ...
    def allocate(
        self,
        signals: pd.Series,
        data: Optional[pd.DataFrame] = None,  # Prices or returns DataFrame
        prices: Optional[pd.DataFrame] = None,  # Deprecated; use data for prices
    ) -> AllocationResult:
        """
