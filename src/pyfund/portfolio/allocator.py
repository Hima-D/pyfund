...
AllocationMethod = Literal["equal_weight", "equal_risk", "risk_parity", "inverse_vol", "kelly", "mean_variance"]
...
class BaseAllocator(ABC):
    ...
    @abstractmethod
    def allocate(self, signals: pd.Series, data: Optional[pd.DataFrame] = None) -> AllocationResult:
        pass
...
class PortfolioAllocator(BaseAllocator):
    ...
    def _equal_risk_contribution(self, returns: pd.DataFrame, active: List[str]) -> Dict[str, float]:
        ...
        try:
            ...
            result = minimize(risk_contribution, x0, args=(cov.values,),
