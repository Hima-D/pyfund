...
class VolatilityModels:
    ...
    def fit(returns, model="GARCH", p=1, q=1, dist="normal"):
        r = returns * 100
        if model == "EGARCH":   m = arch_model(r, vol="EGarch", p=p, o=1, q=q, dist=dist)
        elif model == "GJR":    m = arch_model(r, vol="Garch", p=p, o=1, q=q, dist=dist)
        elif model == "APARCH": m = arch_model(r, vol="APARCH", p=p, q=q, dist=dist)
        else:                   m = arch_model(r, vol="Garch", p=p, q=q, dist=dist)
        res = m.fit(disp="off")
        return {
            "model": model,
            "aic": res.aic,
