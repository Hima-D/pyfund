...
def describe_financial(returns):
    ...
    return {
        ...
        "max_dd": dd.max(),
        "calmar": (equity.iloc[-1]/equity.iloc[0]-1) / dd.max() if dd.max() > 0 else float('inf')
    }
...
def deflated_sharpe_ratio(returns, trials=5000):
    ...
    boot = [np.random.permutation(returns).mean() / np.random.permutation(returns).std() * (252**0.5) 
            for _ in range(trials)]
    boot = [x for x in boot if not np.isnan(x)]
    if not boot: return sr
