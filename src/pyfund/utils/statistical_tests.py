...
class StatisticalValidator:
    ...
    def _sharpe(self, r): 
        ...
        return r.mean() / r.std() * np.sqrt(self.config["annualization_factor"]) if r.std() > 0 else 0.0

    def deflated_sharpe_ratio(self, returns: np.ndarray) -> float:
        returns = np.asarray(returns).flatten()
        if len(returns) < self.config["min_observations"]: return 0.0
        observed = self._sharpe(returns)
        trials = [self._sharpe(self.rng.choice(returns, len(returns))) for _ in range(self.config["dsr_trials"])]
        trials = [t for t in trials if t > 0]
        if not trials: return observed
    ...
    def probability_of_backtest_overfitting(self, equity: np.ndarray) -> float:
        ...
        sample = self.rng.choice(combos, size=min(self.config["pbo_samples"], len(combos)), replace=False)
        matches = 0
        for train_idx in sample:
            ...
            if abs(train_ret.sum()) < 1e-8 or abs(test_ret.sum()) < 1e-8: continue
    ...
    def walk_forward_analysis(self, returns: pd.Series) -> Dict[str, Any]:
        ...
        while i + self.config["wf_test_window"] <= len(returns):
            ...
            if test.std() > 1e-8:
                results.append(test.mean() / test.std() * np.sqrt(self.config["annualization_factor"]))
    ...
    def validate(self, returns: np.ndarray) -> Dict[str, Any]:
        ...
        result["robust"] = (result["deflated_sharpe"] > 1.5 and 
                           result["pbo"] < 0.25 and 
                           result["walk_forward"]["mean"] > 1.5)
        return result
    ...
    def report(self, returns: np.ndarray, title: str = "Strategy Validation Report"):
        ...
        if "error" in r:
            ...
            print(f"{'Deflated Sharpe':<30} {r['deflated_sharpe']:>19.3f} → {'EXCELLENT' if r['deflated_sharpe']>1.5 else 'Weak'}")
            print(f"{'PBO Risk':<30} {r['pbo']:>23.3f} → {'LOW' if r['pbo']<0.25 else 'HIGH'}")
            print(f"{'Walk-Forward Sharpe':<30} {r['walk_forward']['mean']:>15.3f} ± {r['walk_forward']['std']:.3f}")
            print(f"{'WF Periods':<30} {r['walk_forward']['periods']:>19}")
        print("═" * 80)
        verdict = "STRONG EVIDENCE OF PERSISTENT ALPHA" if r.get("robust") else "FURTHER TESTING RECOMMENDED"
