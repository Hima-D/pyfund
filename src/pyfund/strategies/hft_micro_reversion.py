...
class HFTMicroReversionStrategy(BaseStrategy):
    ...
    def __init__(self, ticker: str = "ES", params: Optional[Dict] = None):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        for i in range(self.params["lookback_ticks"], len(data)):
            idx = data.index[i]
            z = z_score.iloc[i]
            vol_ok = volume_spike.iloc[i] > self.params["volume_filter"] or np.isnan(volume_spike.iloc[i])
            ...
            if self.position != 0 and self.entry_time is not None:
                if (current_time - self.entry_time).total_seconds() > self.params["holding_seconds"]:
                    signals.iloc[i] = 0
                    self.position = 0
                    continue
        ...
        logger.info(f"HFT Micro Reversion | Z-score: {z_score.iloc[-1]:.2f} | "
...
if __name__ == "__main__":
    ...
    strategy = HFTMicroReversionStrategy(ticker, {
        "z_entry": 2.0,
        "holding_seconds": 60
    })
