...
class DeltaHedgedStraddleStrategy(BaseStrategy):
    ...
    def __init__(self, ticker: str = "SPY", params: Optional[Dict[str, Any]] = None):
    ...
    def _get_atm_options_chain(self, date: pd.Timestamp) -> Optional[Dict]:
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        if self.position["entry_date"] is None:
            ...
            if last_day.day >= 25 or (last_day - data.index[-30]).days >= 30:
                ...
                if chain and self.params["dte_target"] - self.params["dte_tolerance"] <= chain["dte"] <= self.params["dte_target"] + self.params["dte_tolerance"]:
                    ...
                    logger.info(f"DELTA-HEDGED STRADDLE ENTRY | {self.ticker} | Strike: {chain['strike']} | Cost: ${chain['call_premium'] + chain['put_premium']:.2f} | DTE: {chain['dte']}")
        ...
        if self.position["entry_date"] is not None:
            ...
            current_straddle_value = self.position["straddle_cost"] * decay_factor * 1.1  # + gamma gains
            ...
            self.position["gamma_pnl"] += (current_straddle_value - self.position["straddle_cost"]) * 100

            self.position["total_pnl"] = self.position["gamma_pnl"] + self.position...
            self.position["total_pnl"] = self.position["gamma_pnl"] + self.position["hedge_pnl"] - self.position["straddle_cost"] * 100
            ...
            if days_held >= self.params["dte_target"] or remaining_dte <= 3:
                ...
                logger.info(f"STRADDLE EXIT | P&L: ${self.position['total_pnl']:,.0f} ({pnl_pct:+.1%}) | Gamma: ${self.position['gamma_pnl']:,.0f} | Hedge: ${self.position['hedge_pnl']:,.0f}")
                self.position = {k: None if k in ["entry_date", "expiration"] else 0.0 for k in self.position}
